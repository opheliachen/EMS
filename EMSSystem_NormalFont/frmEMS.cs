...
namespace EMSSystem
{
    public partial class frmEMS : Form
    {
        ...
        public frmEMS()
        {
            ...
            Font newFont = new System.Drawing.Font("MingLiU", 12F, System.Drawing.FontStyle.Bold);
        ...
        private void ReSize()
        {
            ...
            lblSystemTitle.Location = new Point((mainWidth - lblSystemTitle.Width) / 2,
                                                (mainHeight - panelMainGeneralScreen.Height) / 3 - 50);
        ...
        private void ShowPages(int subFunctionsNum, string mainFunctionText, string subFunctionText)
        {
            ...
            if (subFunctionsNum == 0)
            {
                ...
                //    //SetStudentSearchLableButtons(mainFunctionText + "> 課程人次查詢", "應收費用", "應收費用", "", "顯示學生");
                //}
        ...
        public void AfterStudentSearch(string dataID, string dataName)
        {
            ...
            else if (lblCurrentPage.Text.IndexOf("07") > -1)
            {
                ...
                if (btnStudentSearch.Text == "搜 尋")
                {
                    ...
                    btnDeleteExistStudent.Visible = true;
                ...
                else if (btnStudentSearch.Text == "新 增")
                {
                    ...
                    btnDeleteExistStudent.Visible = false;
                    btnRecoveryStudent.Visible = false;
                }
            }
                ...
                if (withDetail)
                {
                    ...
                    else if (lblCurrentPage.Text.IndexOf("11") > -1)
                    {
                        ...
                        else
                        {
                            ...
                            money += recordList.Sum(u => int.Parse(u.Note2));
            ...
            foreach (var recordSingle in recordList)
            {
                ...
                else if (lblCurrentPage.Text.IndexOf("06") > -1)
                {
                    ...
                    if (recordListSets != null && recordListSets.Count > 0)
                        recordListSets = recordListSets.OrderBy(u => u.Date1).ToList();
                ...
                else if (lblCurrentPage.Text.IndexOf("11") > -1)
                {
                    ...
                    if (lblCurrentPage.Text.IndexOf("明細") > -1)
                    {
                        ...
                        //Only Get Input Record
                        tempRecordListSets = tempRecordListSets.Where(u => u.Money1 > 0).ToList();
            ...
            if (!CheckNewStudentErrors())
            {
                ...
                if (lblInvisibleStudentDataStatus.Text.IndexOf("Insert") > -1)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (!string.IsNullOrEmpty(txtNewStudentID.Text.Trim()))
        ...
        private void btnDeleteExistStudent_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("是否確定刪除此學生??", "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (result == DialogResult.Yes)
            {
                facade = new FacadeLayer(SystemTypeForDB);
                string studentID = studentData.ID;
                facade.FacadeFunctions("update", "studentstatus", studentID, "1");

                CreateSystemLogs("刪除學生 " + txtNewStudentName.Text + "(" + studentID + ")");
                MessageBox.Show("修改成功!!!", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);

                studentData = (StudentDefinition)facade.FacadeFunctions("select", "student", (object)"ID", (object)studentID);
                btnRecoveryStudent.Visible = true;
                btnDeleteExistStudent.Visible = false;
            }
        }

        private void btnRecoveryStudent_Click(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("是否確定救回此學生??", "警告", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (result == DialogResult.Yes)
            {
                facade = new FacadeLayer(SystemTypeForDB);
                string studentID = studentData.ID;
                facade.FacadeFunctions("update", "studentstatus", studentID, "0");

                CreateSystemLogs("救回學生 " + txtNewStudentName.Text + "(" + studentID + ")");
                MessageBox.Show("修改成功!!!", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else
            {
                if (lblCurrentPage.Text.IndexOf("刪除") == -1 && isReady)
                {
                    if (lblCurrentPage.Text.IndexOf("全班") == -1)
                    {
                        ...
                        MessageBox.Show("無新班級可加選!!!", "資訊", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            //dgvStudentManageClassNewClass.Refresh();
        ...
        private void ShowDailyExpanseDataForUpdating()
        {
            ...
            dtpExpanseDate.Value = DateTime.Parse(expanseData.InsertDate);
