...
namespace EMSSystem
{
    public partial class frmSearchStudentData : Form
    {
        ...
        private void btnStudentSearch_Click(object sender, EventArgs e)
        {
            ...
            facade = new FacadeLayer(emsSystem.SystemTypeForDB);
            ...
            if (selectBy != null)
            {
                if (cboStudentSearchBy.SelectedItem.ToString().IndexOf("學生") > -1)
                {
                    ...
                    if (lblFromPanel.Text.IndexOf("02") > -1)
                        studentStatus = "studentbydeleted";

                    if (selectBy == "ID")
                        studentData = (StudentDefinition)facade.FacadeFunctions("select", studentStatus, (object)selectBy, (object)selectData);
                    else if (selectBy == "Name")
                    {
                        studentSets = (List<StudentDefinition>)facade.FacadeFunctions("select", studentStatus, (object)selectBy, (object)StaticFunction.SetEncodingString(selectData));
